export default class Line {
  ...
  constructor(rawLine) {
    ...
    this.springs = [springs, springs, springs, springs, springs].join('?')
    this.groups  = []
    this.cache   = {}
  ...
  countTotalArrangements(cache) {
    this.cache = cache
    return this.countArrangements(this.springs, this.groups)
  }

  countArrangements(springs, groups) {
    let cacheKey = JSON.stringify(springs) + "@" + JSON.stringify(groups)
    ...
    if (current == '.') {
      count += this.countArrangements(springs.slice(1), groups)
    }
    ...
    if (current == '#' || current == '?') {
      ...
      if (enoughRoom && noMoreDots) {
        ...
        if (nextSpringAfterGroup != "#") {
          ...
          count += this.countArrangements(remainingSprings, remainingGroups)
        }
      }
    }
