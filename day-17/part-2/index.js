...
class Task {
  ...
  run() {
    this.addNextBlocks('0-0', 'E', 0, 0)
    this.addNextBlocks('0-0', 'S', 0, 0)
  ...
  addNextBlocks(key, direction, distance, heatLoss) {
    ...
    // Can only continue straight if distance less than 10
    if (distance < 10) nextDirections.push(direction)
    ...
    nextDirections.forEach((nextDirection) => {
      ...
      let nextBlock = this.blocks[nextKey]

      if (nextBlock == undefined ) return
      if (nextBlock.isTheEnd  && nextDistance < 4) return
